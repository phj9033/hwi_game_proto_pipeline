# Round 0.5 — Art placeholder + 프롬프트 파일 (auto-build substep)

`auto-build-orchestrator.md` 의 5번째 (마지막) 단계.
...
1. **art-bible §Z 슬롯 맵 파싱**
   ...
   - `category`·`palette_ref` 컬럼 부재 시 (구 형식 art-bible) → LLM 추론으로 보강, art-bible 의 슬롯 행에 두 컬럼 append, `06-changelog.md` 에 1줄:
     ```
     - YYYY-MM-DD v0.5 — art-bible 슬롯 맵 category/palette_ref 보강 (auto-build 호환성)
     ```
...
3. **코드 연결 — 어댑터 *에셋 로드 컨벤션* 적용**
   ...
   - 0.1~0.4 에서 생성된 모듈·시그널·Resource·UI 중 art 슬롯을 참조하는 곳에 경로 박음
   - 0.4 의 §G 데이터 인스턴스 가 art 슬롯을 명시하면 → 해당 인스턴스 Resource 의 art 필드에 자동 wiring
   - 모든 슬롯 wiring 은 무리 — 코어 루프·UX 에 등장하는 슬롯 위주로 자동, 나머지는 미사용 자산으로 art/ 에 대기

4. **smoke 검증**
   - 어댑터의 init_command 재실행 → 빌드 OK 확인 (placeholder 경로 오류 ✕)
   - 실패 시 orchestrator 에 failed_at="0.5" 반환

5. **commit**
   ```
   Round 0.5: art placeholders + prompt files (N slots)
...
6. **ITERATION_LOG append**
   ```
   ## v0.5 — Art placeholder (YYYY-MM-DD)
   **슬롯 수**: N
