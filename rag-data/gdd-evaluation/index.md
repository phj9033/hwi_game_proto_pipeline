...
본 컬렉션은 게임기획서(Game Design Document)의 **평가**와 **작성** 양쪽 작업에 활용 가능한 이론·프레임워크 모음이다. hwicortex 컬렉션 `gdd-evaluation`에 적재되어 RAG로 검색·인용된다.
...
## 컬렉션 적재 가이드(hwicortex)

- 컬렉션 이름: **`gdd-evaluation`**
- 청킹 전략: 섹션 헤더(`##`) 기준 분할 권장. 각 청크에 frontmatter 메타가 자동 부착되도록 인덱싱.
- 임베딩: 한국어·영어 혼용이므로 다국어 임베딩 모델 권장.
- 우선순위: `index.md`는 검색 우선순위를 낮게(navigation 용도), 이론 문서 본문 청크가 우선 회수되도록.
