...
2. **분기 축 결정** — 사용자에게 1회 질문:
   ...
   (b) 지정 — 직접 3개 입력
   ```
3. 자동 선정 시: 3개 후보 + **각각 *왜* 1~2줄 사유** 함께 보여주고 사용자 OK 받기 (1회 멈춤).
   사유는 *서로 의미 있는 차이*가 어느 차원에서 나오는지 명시 (정보 형태·결정 모양·시간 압력·장르 결 등).
4. 3개 드래프트 순차 생성 (`02-draft-A/B/C.md`)
   - 각 드래프트의 §분기 축 섹션에 **"왜 이 변주를 선택했나"** 필드 1~2줄 *필수* 작성 (3가 휘발 ✕)
